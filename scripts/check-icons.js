...
function main() {
  try {
    if (!fs.existsSync(assetsDir)) {
      console.warn(
        `[check-icons] assets directory does not exist: ${assetsDir}`
      );
      console.warn(
        `[check-icons] Create it or update electron-builder icon paths in package.json -> build`
      );
      return;
    }

    const missing = required.filter(
      ({ file }) => !fileExists(path.join(assetsDir, file))
    );
    if (missing.length === 0) {
    ...
    for (const m of missing) {
      console.warn(
        `  - ${m.file} (${m.platform}) -> package.json build key: ${m.builderKey}`
      );
    }
    ...
    console.warn(
      "      macOS (.icns): 512x512 or appiconset converted to .icns"
    );
    console.warn("      Windows (.ico): multi-size 16/32/48/64/128/256");
    console.warn("      Linux (.png): 512x512");
    console.warn("\nQuick options:");
    console.warn(
      "  - Use an online converter to generate .icns and .ico from a 512x512 PNG"
    );
    console.warn(
      "  - Or use electron-icon-builder locally to generate from a source PNG"
    );
    console.warn(
      "    npm i -D electron-icon-builder && npx electron-icon-builder --input=icon.png --output=electron/assets"
    );
