...
import { Clock, Power } from '@phosphor-icons/react';
...
const FinancialCarousel: React.FC<FinancialCarouselProps> = ({
  ...
  const [autoSlideEnabledState, setAutoSlideEnabledState] = useState(autoSlideEnabled);
  ...
  useEffect(() => {
    if (!isAutoSliding || !autoSlideEnabledState) return;
    ...
    return () => clearInterval(interval);
  }, [autoSlideDelay, isAutoSliding, autoSlideEnabledState]);
  ...
  if (isStandbyMode) {
    return (
      <div
        ...
        onClick={() => {
          setAutoSlideEnabledState(false);
          handleWakeUp();
        }}
      >
        ...
        {isAutoSliding && autoSlideEnabledState && !isTransitioning && (
  ...
  // Responsive carousel mode - Show 7 cards on web, 4-6 on mobile depending on viewport
  const totalPositions = 10;

  return (
    <div className={`relative ${className}`}>
      <div className="relative h-80 sm:h-96 lg:h-[28rem]">
        {/* Static Container with all 10 cards */}
        <div className="flex h-full" onTouchStart={handleTouchStart} onTouchMove={handleTouchMove} onTouchEnd={handleTouchEnd}>
          {/* Create 10 static cards, cycling through the 7 slides */}
          {Array.from({ length: totalPositions }, (_, positionIndex) => {
            const slideIndex = positionIndex % slides.length;
            const isActive = slideIndex === currentSlide;

            return (
              <div
                key={`position-${positionIndex}`}
                className={`relative overflow-hidden group cursor-pointer transition-all duration-500 border-r border-white/10 rounded-lg hover:shadow-2xl hover:shadow-primary/20 ${
                  isActive ? 'bg-primary/20' : 'bg-muted/20'
                }`}
                style={{ width: `${100 / totalPositions}%` }}
                onClick={() => goToSlide(slideIndex)}
              >
                {/* Content for each card */}
                <div className={`flex flex-col justify-center h-full p-2 transition-all duration-500 ${
                  isActive ? 'opacity-100' : 'opacity-60'
                }`}>
                  <div className="text-center">
                    <h3 className={`text-xs sm:text-sm font-bold mb-1 leading-tight font-montserrat line-clamp-2 ${
                      isActive ? 'text-primary' : 'text-muted-foreground'
                    }`}>
                      here
                    </h3>
                    <p className={`text-xs font-normal leading-relaxed font-montserrat line-clamp-2 ${
