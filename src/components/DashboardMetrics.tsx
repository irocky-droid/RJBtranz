...
import { getCurrencySymbol } from "./countryData";
...
const DashboardMetrics: React.FC<DashboardMetricsProps> = ({
  ...
  // Calculate revenue growth based on timeframe
  const calculateRevenueGrowth = () => {
    ...
    const currentPeriodTransactions = (transactions || []).filter(t =>
      new Date(t.createdAt) >= currentPeriodStart && new Date(t.createdAt) <= now
    );

    const previousPeriodTransactions = (transactions || []).filter(t =>
      new Date(t.createdAt) >= previousPeriodStart && new Date(t.createdAt) < currentPeriodStart
    );

    const currentRevenue = currentPeriodTransactions.reduce((sum, t) => sum + (t?.fee || 0), 0);
    const previousRevenue = previousPeriodTransactions.reduce((sum, t) => sum + (t?.fee || 0), 0);
        ...
        <>
          ...
          >
            ...
            <CardContent className="p-0 relative z-10">
              <div className="text-2xl font-bold font-dynamic-2xl">{getCurrencySymbol(baseCurrency)} {safeToLocaleString(totalRevenue)}</div>
          ...
          >
            ...
            <CardContent className="p-0 relative z-10">
              <div className="text-2xl font-bold mb-2">
                {getCurrencySymbol(baseCurrency)} {safeToLocaleString(totalVolume)}
              </div>
              ...
              <div className="text-xs text-muted-foreground mb-2">
                Average per transaction: {getCurrencySymbol(baseCurrency)} {safeToLocaleString(
          ...
          >
            ...
            <CardContent className="p-0 relative z-10">
              <div className="text-2xl font-bold">
                {getCurrencySymbol(baseCurrency)} {safeToLocaleString(transactions && transactions.length > 0 ? Math.round(totalVolume / transactions.length) : 0)}
              </div>
