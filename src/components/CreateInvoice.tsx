...
import { COUNTRY_DATA, getCurrencyInfoByCode } from './countryData';

const getCurrencySymbol = (currencyCode: string) => {
  return getCurrencyInfoByCode(currencyCode)?.symbol || currencyCode;
};

const sortedCountryData = [...COUNTRY_DATA].sort((a, b) => {
...
export default function CreateInvoice({ onBack, onComplete, selectedCountry }: CreateInvoiceProps) {
  ...
  const [invoiceData, setInvoiceData] = useState<Invoice>({
    ...
    receiverEmail: '',
    receiverAmount: '',
    receiverCurrency: selectedCountry?.currency || '', // Corrected
    receiverCountry: selectedCountry ? COUNTRY_DATA.find(c => c.currency === selectedCountry.currency)?.code || 'GH' : 'GH',
    receiverPhone: '',
    feeAmount: 0,
    feeRate: 0.0,
    feeCurrency: '',
    feeOnSender: true,
    exchangeRate: selectedCountry?.rate?.rate || 0.00,
    totalFee: 0,
  ...
  const getCountryByCode = (code: string) => {
    return COUNTRY_DATA.find(c => c.code === code) || COUNTRY_DATA[0];
  };
